...
module.exports = angular.module("falcorception.falcorClient", [
  require("ladda/ladda").name,
])

.component("falcorClient", {
  ...
  controller($scope, falcor, falcorFactory) {
    ...
    ctrl.invalidate = function (paths) {
      model.invalidate(paths)
    }

    ctrl.get = co.wrap(function* (paths) {
      yield ctrl.falcorAction("get", paths)
    })

    ctrl.setValue = co.wrap(function* (path, values) {
      yield ctrl.falcorAction("setValue", arguments)
    })

    ctrl.set = co.wrap(function* (update) {
      update = update && JSON.parse(update)
      update = {json: update}
      yield ctrl.falcorAction("set", arguments)
    })
    ...
    ctrl.addPath = function() {
      ctrl.paths.push("")
    }

    ctrl.deletePath = function($index) {
      _.pullAt(ctrl.paths, $index)
    }
  },
})
